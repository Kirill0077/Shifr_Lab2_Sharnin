...
string Get_keyword(int key_len, vector<vector<unsigned char>>table, map<double, unsigned char>freq_standart)
{
	...
	while (num_keysymb < key_len)
	{
		...
		//if (freq_standart.rbegin()->second < freq_shifr.rbegin()->second)
			//keyword.push_back(freq_standart.rbegin()->second - freq_shifr.rbegin()->second);
		//else
		keyword.push_back(freq_shifr.rbegin()->second - freq_standart.rbegin()->second);
		num_keysymb++;
	}
	//код чтобы вывести символы которые идут в порядке возрастания частоты
    /*map <double, unsigned char> ::iterator it = freq_shifr.begin();
	for (int i = 0; it != freq_shifr.end(); it++, i++) {  // выводим их
		cout << i << ") Ключ " << it->first << ", значение " << it->second << endl;
	}
	map <double, unsigned char> ::iterator it1 = freq_standart.begin();
	for (int i = 0; it1 != freq_standart.end(); it1++, i++) {  // выводим их
